...
%% PD gains
approach = 2;
[K_pr, K_pp, K_py, K_dr, K_dp, K_dy] = evaluate_gains(approach); % proportional gains are in N*m, 
K_py = K_py*10;
K_pr=K_pr*1.8;
...
Qattitudefactor=1e-10; %now equal to the ARW^2; tuning if necessary, era 10^-6. 1e-10
Qangvelfactor=1e-7; % tuning if necessary, era 10^-6    1e-7
...
%% Gyros
Bias = deg2rad(1)/3600; % offset short term bias instability, rad per second PG400
ARW = deg2rad(0.23)/sqrt(3600); % Angular Randomic Walk, in rad per second. N.B. : sigma of the angular velocity is ARW * sqrt(dt)
...
R_G = ARW^2; % the sigma on the angular velocity is the ARW divided by sqrt(dt), now dt = 1 so nothing changes
